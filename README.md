...
디버깅 단계에선 Git Api의 사용 한도 초과 에러가 자주 나와서 해당 에러를 명확히 보여주기 위해 서버 에러 메세지 처리를 주로 구현했습니다.
...
### AppWidgetProvider
- Android Native에서 위젯 개발 경험이 있기 때문에, 간단한 네이티브 위젯을 구현했습니다. 
...
- **북마크**
  - 북마크 추가/제거/토글
  - 로컬 기반 데이터 베이스
  - 새로고침 버튼, Swipe Refresh구현.
  - 북마크 전체 삭제 기능
  - 최신순 정렬, 오래된순 정렬
  - 소유자 아이콘은 네트워크가 연결된 상황에서만 볼 수 있음. 네트워크가 연결 안된 경우는 기본 이미지를 표시

- **Android 홈화면 위젯**
  - 최신 북마크 표시
  - RepoWidgetProvider - Kolin기반 구현

- **기타 구현 기능**
  - 상세 속성 창 추가, 정보 조회와 URL 주소 복사 기능 구현
  - 스크롤 아이템이 30 이상이면 상단으로 올라가는 플로팅 버튼 활성화
